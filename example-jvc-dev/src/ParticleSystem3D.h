...
class ParticleSystem3D : public BaseParticleSystem
{
public:
    ...
    void createParticles(int w, int h)
    {
        ...
        for (unsigned y = 0; y < h; ++y)
        {
            for (unsigned x = 0; x < w; ++x)
            {
                ...
                positions[idx * n + 2] = ofRandomf()*100.f; // particle z
    ...
    void loadShaders()
    {
        ...
        updateFrag += STRINGIFY(
                                ...
                                void main()
                                {
                                    ...
                                    vel.x *= step(abs(pos.x), edgeWidth)  * bounceVelocity - 1.0;
                                    vel.y *= step(abs(pos.y), edgeHeight) * bounceVelocity - 1.0;
    ...
    void onParticlesUpdate()
    {
        ...
        updateShader.setUniform1f("edgeWidth", edgeWidth);
        updateShader.setUniform1f("edgeHeight", edgeHeight);
        updateShader.setUniform1f("bounceVelocity", bounceVelocity);
